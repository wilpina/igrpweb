...
public class CargoController extends Controller {
	...
	if(Core.isNotNullOrZero(Core.getParamInt("p_id_cargo"))){
		srstparametrizacaofilter.andWhere("id","=",Core.getParamInt("p_id_cargo"));
	}
	...
	view.btn_gravar.addParameter("p_id_cargo", Core.getParam("p_id_cargo"));
	...
	if (model.validate()) {
		...
		String isEdit = Core.getParam("isEdit");
		if(Core.isNotNull(isEdit)) {
			 srstparametrizacao = session.find(SrsTParametrizacao.class, Core.getParamInt("p_id_cargo"));
		}
		if (srstparametrizacao != null){
			srstparametrizacao.setCodigo(model.getCodigo());
	srstparametrizacao.setDescricao(model.getDescricao() );
	srstparametrizacao.setTipo("CARGO");
